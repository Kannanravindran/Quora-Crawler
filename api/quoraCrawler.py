    ...
    while (current_line):
        ...
        for i in range(1, len(split_html)):
            ...
            for x in range(1, len(users_voted_raw)):
                users_voted_soup = BeautifulSoup(str(users_voted_raw[x]),"xml")
...
def crawlUser():
    ...
    while(current_line):
        ...
        source_soup = BeautifulSoup(html_source, "xml")
        part = source_soup.find_all(attrs={"class":"link_label"})
        part_soup = BeautifulSoup(str(part), "xml")
        ...
        followers_soup = BeautifulSoup(followers_html_source, "xml")
        ...
        for x in range(1, len(followers_raw)):
            followers_soup = BeautifulSoup(str(followers_raw[x]), "xml")
        ...
        following_soup = BeautifulSoup(following_html_source, "xml")
        ...
        for x in range(1, len(following_raw)):
            following_soup = BeautifulSoup(str(following_raw[x]), "xml")
